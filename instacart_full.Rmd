...
lda <- LDA(prod_dtm, k = 5, control = list(seed = seedd))
...
top20 <- inspect(head(sorted_rules, n = 20))
...
for (t in trees){
  for (p in pred){
    rf_model <- rand_forest(mode = "classification", engine = "ranger", 
                            mtry = p, trees = t)
    rf_fit <- rf_model %>% fit(purpose ~ ., data = train) %>% extract_fit_engine()
    error_list <- append(error_list, rf_fit$prediction.error)
    if (rf_fit$prediction.error < best_rf_error){
      best_rf_error = rf_fit$prediction.error
      best_model = rf_fit
    }
  }
}
...
```{r}
best_rf_pred <- predict(best_model, test, type = "response")
#brf_pred <- rep(0, dim(test)[1])
best_rf_pred <- best_rf_pred$predictions[, 2]
best_rf_pred <- ifelse(best_rf_pred >= 0.5, 1, 0)

table(test$purpose, best_rf_pred)/1310*100
```
...
roc_probit = roc(test$purpose ~ prediction_probit, plot = TRUE, print.auc = TRUE, main ="ROC Curve - Logit ")

roc_lasso = roc(test$purpose ~ prediction_lasso, plot = TRUE, print.auc = TRUE, main ="ROC Curve - Lasso")

##Need to add for the other two models
